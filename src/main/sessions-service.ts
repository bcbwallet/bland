...
import { resolve, basename, parse, extname, join } from 'path';
...
export class SessionsService {
  ...
  public async loadExtensions() {
    ...
    const bundledExtensionsPath = join(app.getAppPath(), 'build/extensions').replace('app.asar', 'app.asar.unpacked');
    const bundledExtensionsDirs = await promises.readdir(bundledExtensionsPath);
    paths = bundledExtensionsDirs.map(dir => resolve(bundledExtensionsPath, dir));

    const extensionsPath = getPath('extensions');
    const extensionsDirs = await promises.readdir(extensionsPath);
    paths = paths.concat(extensionsDirs.map(dir => resolve(extensionsPath, dir)));

    for (const path of paths) {
      try {
        console.log('Load extension from: ', path)
